  ...
  ({ playlist, currentInstanceUri }) => {
    ...
    return (
      ...
      >
        ...
        <Badge
          color={isRemotePlaylist ? "blue" : (playlist as any).type === "cache" || playlist.title === "Cache" ? "cyan" : "green"}
          variant="light"
          mb="sm"
        >
          {isRemotePlaylist ? "Remote" : (playlist as any).type === "cache" || playlist.title === "Cache" ? "Cache" : "Local"}
        </Badge>
        <Text
          size="xs"
          lineClamp={2}
