...
export default (() => {
  ...
  if (!migrationsName.includes("19102023_migrate_playlist_favorite_cards_key")
  ) {
    try {
  ...
  if (!migrationsName.includes("01032026_migrate_device_musical_names")) {
    try {
      require("./01032026_migrate_device_musical_names").default();
